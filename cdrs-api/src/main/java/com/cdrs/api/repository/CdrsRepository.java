...
public interface CdrsRepository extends JpaRepository<CdrsDbDetails, Long> {
    ...
    @Query(value="SELECT sum(t.actual_used_amount)  as actual_duration,sum(t.used_amount*60) as rounded_duration,sum(t.charge) as charge ,t.file_name as file FROM cdrs_details t where t.service_type = 1 group by t.file_name", nativeQuery = true)
    ...
    @Query(value="select * from cdrs_details o  GROUP by DAY(o.start_datetime) , HOUR(o.start_datetime)", nativeQuery = true)
    List<CdrsDbDetails> getChargePerHourByDay();
